...
function ViewContact(props) {
    ...
    const assingToTaskContact = (email) => {
        debugger
        props.setIndexContact(props.index)
    ...
    return (
        <>
            <div className="option-contact row mb-2" >
                ...
                <p onClick={(e) => assingToTaskContact(props.contact.email)} className="name-contact ">{props.contact.name} </p>   <p className="email-contact ">{props.contact.email} </p></div>
        </>
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        setIndexContact: (index) => dispatch(actions.setIndexContact(index))
